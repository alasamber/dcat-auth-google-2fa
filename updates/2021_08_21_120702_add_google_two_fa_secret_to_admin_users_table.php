...
class AddGoogleTwoFaSecretToAdminUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::table($this->config('database.users_table'), function (Blueprint $table) {
            if (Schema::hasColumns($this->config('database.users_table'), ['google_two_fa_secret', 'google_two_fa_enable', 'status'])) {
                return;
            }
            ...
            $table->boolean('status')->default(1)->after('google_two_fa_enable');
        });
    }
